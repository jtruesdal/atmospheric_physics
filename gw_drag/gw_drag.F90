...
!--------------------------------------------------------------------------
  ...
  real(kind_phys),pointer   :: pref_edge(:), pref_mid(:)
...
!!$    gw_rdg_do_vdiff,  &
  ...
  real(kind_phys), pointer, intent(in)     :: pref_edge_in(:)
  real(kind_phys), pointer, intent(in)     :: pref_mid_in(:)
...
!!$  logical, intent(in)             ::  gw_rdg_do_vdiff
  ...
  cm_desc_frontgfc = cm_desc_frontgfc
...
subroutine gw_drag_run( &
  ...
  real(kind_phys), intent(in) :: epsilo
  ...
  real(kind_phys), intent(in) :: state_s(:,:)   ! dry static energy
...
!!$  real(kind_phys), parameter :: dlat0 = 5.0_kind_phys * degree2radian
  ...
  real(kind_phys) :: dse(ncol,pver)
  real(kind_phys) :: flx_heat_curr(ncol)

  !------------------------------------------------------------------------
  beres_dp_desc%storm_shift = beres_dp_desc_storm_shift
  beres_dp_desc%k = beres_dp_desc_k
  beres_dp_desc%min_hdepth = beres_dp_desc_min_hdepth
  beres_dp_desc%maxh = beres_dp_desc_maxh
  beres_dp_desc%maxuh = beres_dp_desc_maxuh
  beres_dp_desc%hd = beres_dp_desc_hd
  beres_dp_desc%mfcc = beres_dp_desc_mfcc
  beres_sh_desc%storm_shift = beres_sh_desc_storm_shift
  beres_sh_desc%k = beres_sh_desc_k
  beres_sh_desc%min_hdepth = beres_sh_desc_min_hdepth
  beres_sh_desc%maxh = beres_sh_desc_maxh
  beres_sh_desc%maxuh = beres_sh_desc_maxuh
  beres_sh_desc%hd = beres_sh_desc_hd
  beres_sh_desc%mfcc = beres_sh_desc_mfcc
  movmtn_desc%storm_shift = movmtn_desc_storm_shift
  movmtn_desc%k = movmtn_desc_k
  movmtn_desc%min_hdepth = movmtn_desc_min_hdepth
  movmtn_desc%maxh = movmtn_desc_maxh
  movmtn_desc%maxuh = movmtn_desc_maxuh
  movmtn_desc%hd = movmtn_desc_hd
  movmtn_desc%uh = movmtn_desc_uh
  movmtn_desc%mfcc = movmtn_desc_mfcc
  cm_desc%ksrc = cm_desc_ksrc
  cm_desc%kfront = cm_desc_kfront
  cm_desc%frontgfc = cm_desc_frontgfc
  cm_desc%src_tau = cm_desc_src_tau
  cm_igw_desc%ksrc = cm_igw_desc_ksrc
  cm_igw_desc%kfront = cm_igw_desc_kfront
  cm_igw_desc%frontgfc = cm_igw_desc_frontgfc
  cm_igw_desc%src_tau = cm_igw_desc_src_tau
  ...
  if (use_gw_movmtn_pbl) then
     ...
     call gw_movmtn_run(ncol, band_movmtn , movmtn_desc, state_t, pcnst, &
...
!!$     call outfld ('SMAXQ0', maxq0, ncol, lchnk)
  ...
  if (use_gw_front) then
     ...
     call gw_cm_src(ncol, band_mid, cm_desc, state_u, state_v, frontgf(:ncol,:), &
...
!!$     deallocate(tau, gwut, phase_speeds)
  ...
  if (use_gw_front_igw) then
     ...
     call gw_cm_src(ncol, band_long, cm_igw_desc, state_u, state_v, frontgf(:ncol,:), &
          src_level, tend_level, tau, ubm, ubi, xv, yv, phase_speeds)
...
!!$     call outfld('ZMGW', zm , ncol, lchnk)
  ...
  if (use_gw_rdg_gamma) then
     ...
     hwdthg => rdg_hwdthg(:ncol,:)
     clngtg => rdg_clngtg(:ncol,:)
     gbxar  => rdg_gbxar(:ncol)
     mxdisg => rdg_mxdisg(:ncol,:)
     angllg => rdg_angllg(:ncol,:)
     anixyg => rdg_anixyg(:ncol,:)
