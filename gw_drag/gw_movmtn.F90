...
use shr_log_mod, only: errorMsg => shr_log_errMsg
use gw_common, only: pver, GWBand, gw_prof, gw_drag_prof, calc_taucd
...
!==========================================================================
subroutine gw_movmtn_init(file_name, band, desc,gw_dc, wavelength, masterproc, iulog, errmsg, errflg )
  ...
  integer :: mfccid, uhid, hdid, stat,iulog
 ...
 desc%maxuh = 241 ! get_pio_dimlen(gw_file_desc, "MW", file_path)
  ...
  call shr_assert(stat == 0, &
       "gw_movmtn_init: Allocation error (hd): "//msg// &
 ...
 ! Allocate wind and get data.
  ...
  call shr_assert(stat == 0, &
       "gw_movmtn_init: Allocation error (uh): "//msg// &
       errorMsg(__FILE__, __LINE__))
  ...
  call shr_assert(stat == 0, &
       "gw_movmtn_init: Allocation error (mfcc): "//msg// &
       errorMsg(__FILE__, __LINE__))
...
!==========================================================================
subroutine gw_movmtn_run(ncol, band, desc, state_t, pcnst, &
...
!---------------------------Local Storage-------------------------------
  ...
  call alloc_err(stat,'gw_tend','tau',ncol*(band%ngwv**2+1)*(pver+1))
  allocate(gwut(ncol,pver,-band%ngwv:band%ngwv),stat=stat)
  call alloc_err(stat,'gw_tend','gwut',ncol*pver*band%ngwv**2+1)
  allocate(phase_speeds(ncol,-band%ngwv:band%ngwv),stat=stat)
  call alloc_err(stat,'gw_tend','phase_speeds',ncol*band%ngwv**2+1)
  ...
  call gw_movmtn_src(ncol, band, desc, &
...
!!$     call outfld('HDEPTH_MOVMTN', hdepth/1000._kind_phys, ncol, lchnk)
...
!---------------------------Local Storage-------------------------------
  ...
  vsteer = v(:,Steer_k)
  steer_level = real(Steer_k,kind_phys)
